...
def create_kitti_submission(model, output_path='./kitti_submission'):
    ...
    for test_id in range(len(test_dataset)):
        ...
        flow_pr = model.module(image1, image2, iters = 20)
        #print(flow_pr[0].shape, image1.shape, padder.unpad(flow_pr[0]).shape)
        flow = padder.unpad(flow_pr[0]).squeeze(0).permute(1, 2, 0).cpu().numpy()
